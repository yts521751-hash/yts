...
function Bias({ value }: { value: number | null }) {
  ...
  const up = value >= 0;
  return (
    <span className={up ? "text-[var(--mk-surge)]" : "text-[var(--mk-exit)]"}>
      {up ? "+" : ""}
      {value.toFixed(2)}%
    </span>
  );
...
export default function MaScreenerPage() {
  const [data, setData] = useState<MaScreenerPayload | null>(null);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [filter, setFilter] = useState<Filter>("all3");
  ...
  return (
    <div className="relative min-h-full flex-1 pb-20 md:pb-6">
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        {error ? (
          <div className="mt-6 rounded-xl border border-[var(--mk-exit)]/30 bg-[var(--mk-exit)]/10 px-4 py-3 text-sm text-[var(--mk-exit)]">
            {error}
          </div>
        ) : null}
