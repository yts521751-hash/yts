...
const TERMS = [
  ...
  },
  {
    id: "wind",
    title: "風度儀表板是什麼？",
    body: "風力度依「指數相對 MA5／月／季線」細分計分：站上各線權重大、跌破權重小，因此「站上月季線」與「跌破月季線」不會再打成同分。右上角標籤另標結構（含低於五日、僅站上季線等）。分級：強風＝多頭或空頭排列完整；陣風＝半成形；亂流＝均線打架且波動高；無風＝糾結且波動低。金流 active 在每日 18:00 灰度切換前維持上個交易日收盤；開頁若已有 active 不會再顯示「背景同步中」。",
  },
  {
    id: "cache",
    title: "為什麼第一次很慢、之後較快？",
    body: "伺服器永遠先讀 active（上個交易日定稿）；平日 18:00 前重建只寫 staging，18:00／18:30／19:00 才灰度切 active。只有實際在讀 staging（尚無正式 active）才會標「同步中」。瀏覽器 stale-while-revalidate：先進完整本機快取再靜默核對，不再把背景重建誤標成資料不完整。",
  },
  {
    id: "fear",
    title: "波動情緒指標怎麼算？",
  ...
  },
  {
    id: "turnover",
    title: "成交金額排行",
    body: "與資金流無關的單純排行：把當日上市＋上櫃個股依成交金額（億元）由高到低排列。盤中約每 3 秒即時更新；休市／週末固定上個交易日結果，不持續刷新。",
  },
  {
    id: "news",
    title: "熱議新聞怎麼更新？",
  ...
  },
  {
    id: "gray",
    title: "灰度同步是什麼？",
    body: "行情／法人平日 18:00／18:30／19:00 先寫 staging，驗證後才原子切 active（灰度）；18:00 前網頁一律讀上個交易日 active，開頁即有正確資料。新聞每 10 分鐘 staging→active。force 只觸發背景重建，不卡住畫面。",
  },
];
